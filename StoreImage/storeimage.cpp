...
// Store Color Images
    ...
    if(DH->Modules.Classifier.active)
    {
        std::string ColorImageName = DH->StoreImageAddress.ImageName + DH->StoreImageAddress.ImageSuffix + ".jpg";
    ...
    {
        // Store plate Images
        std::string PlateImageName = DH->StoreImageAddress.ImageName + DH->StoreImageAddress.ImageSuffix + ".jpg";
...
bool storeimage::CreateAddress(const std::shared_ptr<DataHandler::DataHandlerStruct> &DH)
{
    ...
    if(DH->Modules.CheckOperator.active)
    {
        ...
        DH->StoreImageAddress.ImageSuffix = "_P";
    }else if(DH->Modules.Classifier.active)
    {
        DH->StoreImageAddress.ImageFolder = std::to_string(DH->ProcessedInputData.VehicleModel)+ "/" + Year + "/" + Month + "/" + Day + "/" + Hour + "/" + Min + "/" + Sec + "/";//std::to_string(DH->Input.DeviceID) + "/" + Year + "/" + Month + "/" + Day + "/" + Hour + "/" + Min + "/" + Sec + "/"; //+ DH->ProcessedInputData.MongoID + "/";//TODO mongoID
        DH->StoreImageAddress.ImageSuffix = "_I";

    }
        DH->StoreImageAddress.ImageName = DH->Input.MasterPlate + "_" +std::to_string(DH->Input.Probability);
        DH->StoreImageAddress.ImageAddress = DH->StoreImageAddress.ImageFolder + DH->StoreImageAddress.ImageName + DH->StoreImageAddress.ImageSuffix + ".jpg";//TODO when add colorImage ans banner uncomment this 
